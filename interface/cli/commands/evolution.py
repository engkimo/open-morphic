...
from interface.cli.formatters import (
    console,
    print_engine_preference_table,
    print_error,
    print_model_preference_table,
    print_recovery_rule_table,
)
...
def strategies(
    kind: str | None = typer.Option(
        None, "--type", "-t", help="Filter: model, engine, recovery"
    ),
) -> None:
    """Show learned strategy preferences and recovery rules."""
    c = _get_container()

    show_models = kind is None or kind == "model"
    show_engines = kind is None or kind == "engine"
    show_recovery = kind is None or kind == "recovery"

    found = False

    if show_models:
        prefs = c.strategy_store.load_model_preferences()
        if prefs:
            found = True
            print_model_preference_table(prefs)
        elif kind == "model":
            console.print("[dim]No model preferences learned yet.[/dim]")

    if show_engines:
        prefs = c.strategy_store.load_engine_preferences()
        if prefs:
            found = True
            print_engine_preference_table(prefs)
        elif kind == "engine":
            console.print("[dim]No engine preferences learned yet.[/dim]")

    if show_recovery:
        rules = c.strategy_store.load_recovery_rules()
        if rules:
            found = True
            print_recovery_rule_table(rules)
        elif kind == "recovery":
            console.print("[dim]No recovery rules learned yet.[/dim]")

    if not found and kind is None:
        console.print(
            "[dim]No strategies learned yet. "
            "Run `morphic evolution update` to learn from execution history.[/dim]"
        )
