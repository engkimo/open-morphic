...
class JsonlAuditLogger(AuditLogger):
    ...
    def get_produced_files(self, since: datetime | None = None) -> list[str]:
        """Extract file paths from successful fs_write entries.

        Returns a deduplicated list of paths created since the given time.
        """
        entries = self.query(tool="fs_write", since=since)
        paths: list[str] = []
        seen: set[str] = set()
