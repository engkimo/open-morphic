...
class OllamaEngineDriver(AgentEnginePort):
    ...
    async def run_task(
        ...
        # TD-098: Reject non-Ollama models so the engine chain falls through
        # to ReactExecutor / LiteLLM API instead of silently using qwen3.
        ollama_name = resolved.removeprefix("ollama/")
        installed = await self._ollama.list_models()
        if not any(ollama_name in m for m in installed):
            return AgentEngineResult(
                engine=AgentEngineType.OLLAMA,
                success=False,
                output="",
                error=f"Model {ollama_name} not installed in Ollama",
                model_used=resolved,
            )
