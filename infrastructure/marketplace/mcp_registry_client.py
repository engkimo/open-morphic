...
class MCPRegistryClient(ToolRegistryPort):
    ...
    def _parse_item(self, item: dict[str, Any]) -> ToolCandidate:
        ...
        if not source_url and isinstance(repo, dict):
            source_url = repo.get("url", "")

        # Extract transport from remotes array (v0.1 format)
        transport = item.get("transport", "stdio")
        remotes = item.get("remotes", [])
        if remotes and isinstance(remotes[0], dict):
            transport = remotes[0].get("type", transport)
