...
class TaskEntity(BaseModel):
    ...
    def get_ready_subtasks(self) -> list[SubTask]:
        ...
        completed_ids = {
            ...
            if t.status in (SubTaskStatus.SUCCESS, SubTaskStatus.DEGRADED)
        }
        return [
            t
            for t in self.subtasks
    ...
    def is_complete(self) -> bool:
        terminal = (SubTaskStatus.SUCCESS, SubTaskStatus.FAILED, SubTaskStatus.DEGRADED)
    ...
    def success_rate(self) -> float:
        if not self.subtasks:
            return 0.0
        succeeded = sum(
            1 for t in self.subtasks if t.status in (SubTaskStatus.SUCCESS, SubTaskStatus.DEGRADED)
        )
