...
class ExecuteTaskUseCase:
    ...
    async def execute(
        ...
        # Wire SSE event bus + task repo for intermediate persistence
        if hasattr(self._engine, "_event_bus"):
            self._engine._event_bus = self._event_bus  # type: ignore[union-attr]
            self._engine._task_repo = self._repo  # type: ignore[union-attr]
        ...
        # Skill acquisition loop: discover → install → retry (max once)
        if result.status in (TaskStatus.FAILED, TaskStatus.FALLBACK):
            acquired = await self._try_acquire_skills(result)
            if acquired and self._max_skill_retries > 0:
                logger.info(
                    "Skill acquired — retrying task %s", task_id,
                )
                t0 = time.monotonic()
                result = await self._engine.execute(result)
                duration_s = time.monotonic() - t0
        ...
        # SSE: emit task_completed AFTER final status + cost are persisted
        if self._event_bus is not None and hasattr(self._event_bus, "publish"):
            self._event_bus.publish(result.id, {
                "type": "task_completed",
    ...
    async def _try_acquire_skills(self, task: TaskEntity) -> bool:
        """Discover missing tools, install the best candidate, return True if acquired.

        Closes the discover → install → retry loop. Only installs tools with
        safety tier >= COMMUNITY. Returns False if nothing was installed.
        """
        ...
        try:
            ...
            # No installer wired → log-only (legacy behavior)
            ...
            safe = [
                ...
                if s.safety_tier >= SafetyTier.COMMUNITY
            ]
            if not safe:
                logger.info(
                    "No safe tool candidates (>= COMMUNITY) for task %s",
                    task.id,
                )
                return False

            # Install the top candidate
            best = safe[0]
            logger.info(
                "Installing tool '%s' (safety=%s) for task %s",
                best.name,
                best.safety_tier.name,
                task.id,
            )
            install_result = await self._install_tool.install(best)
            if install_result.success:
                logger.info("Tool '%s' installed successfully", best.name)
                return True

            logger.warning(
                "Tool '%s' install failed: %s", best.name, install_result.message,
            )
            return False
        except Exception:
            logger.warning(
                "Skill acquisition failed for task %s — continuing",
                task.id,
                exc_info=True,
            )
            return False
