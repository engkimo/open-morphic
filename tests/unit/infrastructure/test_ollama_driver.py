...
def mock_ollama():
    ...
    om.list_models = AsyncMock(return_value=["qwen3:8b", "deepseek-r1:8b"])
    return om
...
class TestRunTask:
    ...
    @pytest.mark.asyncio
    async def test_rejects_non_ollama_model(self, driver, mock_ollama):
        """TD-098: Non-Ollama models (e.g. o4-mini) are rejected immediately."""
        mock_ollama.list_models.return_value = ["qwen3:8b"]
        result = await driver.run_task("test", model="o4-mini")
        assert result.success is False
        assert "not installed" in result.error

    @pytest.mark.asyncio
    async def test_accepts_installed_model(self, driver, mock_gateway, mock_ollama):
        """TD-098: Installed Ollama models are accepted."""
        mock_ollama.list_models.return_value = ["qwen3:8b", "llama3:8b"]
        mock_gateway.complete.return_value = LLMResponse(
            content="ok",
        ...
        result = await driver.run_task("test", model="llama3:8b")
        assert result.success is True
