...
class TestRunTask:
    ...
    async def test_full_success_flow(self, driver):
        """Create conversation → poll → completed."""
    ...
    async def test_create_error(self, driver):
        ...
        with patch.object(driver, "_request", new_callable=AsyncMock, return_value=create_resp):
    ...
    async def test_create_request_exception(self, driver):
        with patch.object(
            driver, "_request", new_callable=AsyncMock, side_effect=Exception("Network error")
        ):
            result = await driver.run_task("test")
        assert result.success is False
    ...
    async def test_model_passing(self, driver):
        ...
        async def mock_request(method, path, **kwargs):
            if method == "post":
                # Verify model is passed in create request
                assert kwargs.get("json", {}).get("model") == "gpt-4o"
                return create_resp
            return poll_resp
    ...
    async def test_default_model(self, driver):
        ...
        async def mock_request(method, path, **kwargs):
            if method == "post":
                assert kwargs.get("json", {}).get("model") == "claude-sonnet-4-6"
                return create_resp
            return poll_resp
