...
class TestRunEngine:
    ...
    def test_run_includes_fallback_fields(self, client: TestClient) -> None:
        """BUG-003: Response should include fallback transparency fields."""
        resp = client.post("/api/engines/run", json={"task": "Quick question"})
        data = resp.json()
        assert "fallback_reason" in data
        assert "engines_tried" in data
        assert "fallback_attempts" in data
        assert isinstance(data["engines_tried"], list)
        assert isinstance(data["fallback_attempts"], list)

    def test_run_fallback_attempt_structure(
        self, client: TestClient, container: _MockContainer
    ) -> None:
        """BUG-003: Fallback attempts have engine, attempted, skip_reason fields."""
        resp = client.post("/api/engines/run", json={"task": "Quick question"})
        data = resp.json()
        for attempt in data["fallback_attempts"]:
            assert "engine" in attempt
            assert "attempted" in attempt
            assert "skip_reason" in attempt
