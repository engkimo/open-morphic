...
# ═══════════════════════════════════════════════════════════════
# BUG-003: Fallback attempt tracking
# ═══════════════════════════════════════════════════════════════


class TestFallbackAttemptTracking:
    """BUG-003: AgentEngineResult must expose which engines were tried and why."""

    async def test_success_on_first_engine_has_attempt(self) -> None:
        """Single engine success: engines_tried populated, no fallback_reason."""
        drivers = {AgentEngineType.OLLAMA: _make_driver(AgentEngineType.OLLAMA)}
        uc = RouteToEngineUseCase(drivers)
        result = await uc.execute("task", preferred_engine=AgentEngineType.OLLAMA)
        assert result.success is True
        assert len(result.fallback_attempts) == 1
        assert result.fallback_attempts[0].engine == "ollama"
        assert result.fallback_attempts[0].attempted is True
        assert result.fallback_reason is None

    async def test_fallback_records_unavailable(self) -> None:
        """Unavailable engine should appear as unattempted with skip_reason."""
        claude = _make_driver(AgentEngineType.CLAUDE_CODE, available=False)
        ollama = _make_driver(AgentEngineType.OLLAMA)
        drivers = {AgentEngineType.CLAUDE_CODE: claude, AgentEngineType.OLLAMA: ollama}
        uc = RouteToEngineUseCase(drivers)
        result = await uc.execute(
            "task",
            preferred_engine=AgentEngineType.CLAUDE_CODE,
        ...
        # First: claude_code unavailable
        assert result.fallback_attempts[0].engine == "claude_code"
        assert result.fallback_attempts[0].attempted is False
        assert result.fallback_attempts[0].skip_reason == "unavailable"
        # Second: ollama succeeded
        assert result.fallback_attempts[1].engine == "ollama"
        assert result.fallback_attempts[1].attempted is True
        # Fallback reason should mention claude_code
        assert result.fallback_reason is not None
        assert "claude_code" in result.fallback_reason

    async def test_fallback_records_exception(self) -> None:
        """Engine that raises exception should appear with skip_reason=exception."""
        claude = _make_driver(AgentEngineType.CLAUDE_CODE)
        claude.run_task = AsyncMock(side_effect=TimeoutError("timeout"))
        ollama = _make_driver(AgentEngineType.OLLAMA)
        drivers = {AgentEngineType.CLAUDE_CODE: claude, AgentEngineType.OLLAMA: ollama}
        uc = RouteToEngineUseCase(drivers)
        result = await uc.execute(
        ...
        assert result.success is True
        assert result.engine == AgentEngineType.OLLAMA
        exc_attempt = result.fallback_attempts[0]
        assert exc_attempt.engine == "claude_code"
        assert exc_attempt.attempted is True
        assert exc_attempt.skip_reason == "exception"
        assert "timeout" in (exc_attempt.error or "")

    async def test_fallback_records_failure(self) -> None:
        """Engine that returns success=False should appear with skip_reason=failure."""
        claude = _make_driver(AgentEngineType.CLAUDE_CODE, success=False, error="model overloaded")
        ollama = _make_driver(AgentEngineType.OLLAMA)
        drivers = {AgentEngineType.CLAUDE_CODE: claude, AgentEngineType.OLLAMA: ollama}
        uc = RouteToEngineUseCase(drivers)
        result = await uc.execute(
        ...
        assert result.success is True
        fail_attempt = result.fallback_attempts[0]
        assert fail_attempt.skip_reason == "failure"
        assert "overloaded" in (fail_attempt.error or "")

    async def test_all_fail_returns_attempts(self) -> None:
        """When all engines fail, result still includes full attempt history."""
        claude = _make_driver(AgentEngineType.CLAUDE_CODE, available=False)
        ollama = _make_driver(AgentEngineType.OLLAMA, success=False, error="OOM")
        drivers = {AgentEngineType.CLAUDE_CODE: claude, AgentEngineType.OLLAMA: ollama}
        uc = RouteToEngineUseCase(drivers)
        result = await uc.execute(
            "task",
    ...
    async def test_engines_tried_lists_full_chain(self) -> None:
        """engines_tried should list all engines in the routing chain."""
        drivers = _make_drivers()
        uc = RouteToEngineUseCase(drivers)
        result = await uc.execute("task")
        assert result.success is True
        assert len(result.engines_tried) >= 1

    async def test_no_engines_available_has_reason(self) -> None:
        """Empty driver dict: result should have fallback_reason."""
        uc = RouteToEngineUseCase({})
        result = await uc.execute("task")
        assert result.success is False
        assert result.fallback_reason == "no_engines_available"

    async def test_not_registered_engine_tracked(self) -> None:
        """Engine in chain but not in drivers should appear as not_registered."""
        # Only register ollama, but prefer claude_code
        drivers = {AgentEngineType.OLLAMA: _make_driver(AgentEngineType.OLLAMA)}
        uc = RouteToEngineUseCase(drivers)
        result = await uc.execute(
            "task",
        ...
        assert result.success is True
        # claude_code should be not_registered
        not_reg = [a for a in result.fallback_attempts if a.engine == "claude_code"]
        assert len(not_reg) == 1
        assert not_reg[0].skip_reason == "not_registered"
...
class TestEngineCostRecording:
    """BUG-002: Successful engine execution must record cost via CostTracker."""

    async def test_cost_recorded_on_success(self) -> None:
        """CostTracker.record_engine_result called when engine succeeds."""
        cost_tracker = AsyncMock()
        cost_tracker.record_engine_result = AsyncMock()
        drivers = {AgentEngineType.OLLAMA: _make_driver(AgentEngineType.OLLAMA, cost_usd=0.05)}
        uc = RouteToEngineUseCase(drivers, cost_tracker=cost_tracker)
        result = await uc.execute("task")
        assert result.success is True
        cost_tracker.record_engine_result.assert_awaited_once()
        recorded = cost_tracker.record_engine_result.call_args[0][0]
        assert recorded.cost_usd == 0.05

    async def test_cost_not_recorded_on_failure(self) -> None:
        """CostTracker should not be called when all engines fail."""
        cost_tracker = AsyncMock()
        cost_tracker.record_engine_result = AsyncMock()
        drivers = {
            AgentEngineType.OLLAMA: _make_driver(
                AgentEngineType.OLLAMA, success=False, error="fail"
            )
        }
        uc = RouteToEngineUseCase(drivers, cost_tracker=cost_tracker)
        result = await uc.execute("task")
        assert result.success is False
        cost_tracker.record_engine_result.assert_not_awaited()

    async def test_cost_recording_error_swallowed(self) -> None:
        """Cost recording failure must not break execution."""
        cost_tracker = AsyncMock()
        cost_tracker.record_engine_result = AsyncMock(side_effect=RuntimeError("DB down"))
        drivers = {AgentEngineType.OLLAMA: _make_driver(AgentEngineType.OLLAMA)}
        uc = RouteToEngineUseCase(drivers, cost_tracker=cost_tracker)
        result = await uc.execute("task")
        assert result.success is True

    async def test_no_cost_tracker_no_error(self) -> None:
        """No cost_tracker provided: execution succeeds without error."""
        drivers = {AgentEngineType.OLLAMA: _make_driver(AgentEngineType.OLLAMA)}
        uc = RouteToEngineUseCase(drivers)
        result = await uc.execute("task")
        assert result.success is True

    async def test_cost_recording_with_fallback(self) -> None:
        """Only the successful engine's cost should be recorded."""
        cost_tracker = AsyncMock()
        cost_tracker.record_engine_result = AsyncMock()
        claude = _make_driver(AgentEngineType.CLAUDE_CODE, available=False)
        ollama = _make_driver(AgentEngineType.OLLAMA, cost_usd=0.0)
        drivers = {AgentEngineType.CLAUDE_CODE: claude, AgentEngineType.OLLAMA: ollama}
        uc = RouteToEngineUseCase(
            drivers,
        ...
        cost_tracker.record_engine_result.assert_awaited_once()
        recorded = cost_tracker.record_engine_result.call_args[0][0]
        assert recorded.engine == AgentEngineType.OLLAMA
